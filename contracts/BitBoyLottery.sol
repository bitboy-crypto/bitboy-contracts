...
// File: contracts/interfaces/IBitBoyLottery.sol

pragma solidity ^0.8.4;

interface IBitBoyLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in BITBOY per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in BITBOY token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInBitBoy: price of a ticket in BITBOY
...
// File: contracts/BitBoyLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title BitBooy Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract BitBoyLottery is ReentrancyGuard, IBitBoyLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBitBoy = 500 ether;
    uint256 public minPriceTicketInBitBoy = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInBitBoy;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] bitBoyPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInBitBoy;
        uint32 finalNumber;
    }
     ...
     * @param _bitBoyTokenAddress: address of the BITBOY token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _bitBoyTokenAddress, address _randomGeneratorAddress) {
        bitBoyToken = IERC20(_bitBoyTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of BITBOY to this contract
        uint256 amountBitBoyToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInBitBoy,
            _ticketNumbers.length
        );

        // Transfer BITBOY tokens to this contract
        uint256 balanceBefore = bitBoyToken.balanceOf(address(this));
        bitBoyToken.safeTransferFrom(address(msg.sender), address(this), amountBitBoyToTransfer);
        amountBitBoyToTransfer = bitBoyToken.balanceOf(address(this)).sub(balanceBefore);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInBitBoy += amountBitBoyToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInBitBoyToTransfer
        uint256 rewardInBitBoyToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInBitBoyToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        bitBoyToken.safeTransfer(msg.sender, rewardInBitBoyToTransfer);

        emit TicketsClaim(msg.sender, rewardInBitBoyToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInBitBoy) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in BITBOY for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].bitBoyPerBracket[j] =
                ...
                // A. No BITBOY to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].bitBoyPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInBitBoy - amountToShareToWinners);

        // Transfer BITBOY to treasury address
        if (amountToWithdrawToTreasury > 0) {
            bitBoyToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
        }
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        uint256 balanceBefore = bitBoyToken.balanceOf(address(this));
        bitBoyToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _amount = bitBoyToken.balanceOf(address(this)).sub(balanceBefore);

        _lotteries[_lotteryId].amountCollectedInBitBoy += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInBitBoy >= minPriceTicketInBitBoy) && (_priceTicketInBitBoy <= maxPriceTicketInBitBoy),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInBitBoy: _priceTicketInBitBoy,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            bitBoyPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInBitBoy: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(bitBoyToken), "Cannot be BITBOY token");
    ...
    /**
     * @notice Set BITBOY price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInBitBoy: minimum price of a ticket in BITBOY
     * @param _maxPriceTicketInBitBoy: maximum price of a ticket in BITBOY
     */
    function setMinAndMaxTicketPriceInBitBoy(uint256 _minPriceTicketInBitBoy, uint256 _maxPriceTicketInBitBoy)
        external
        onlyOwner
    {
        require(_minPriceTicketInBitBoy <= _maxPriceTicketInBitBoy, "minPrice must be < maxPrice");

        minPriceTicketInBitBoy = _minPriceTicketInBitBoy;
        maxPriceTicketInBitBoy = _maxPriceTicketInBitBoy;
    }
     ...
     * @param _priceTicket price of a ticket (in BITBOY)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].bitBoyPerBracket[_bracket];
        } else {
            return 0;
        }
    }
